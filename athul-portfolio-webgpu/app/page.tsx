...
export default function Page() {
  ...
  useEffect(() => {
    ...
    const dots = Array.from(
      document.querySelectorAll<HTMLAnchorElement>(".side-dots .dot")
    );
    const onScroll = () => {
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{ minWidth: 220 }}
      >
        <div className="mb-3 text-[0.7rem] uppercase tracking-[0.2em] text-cyan-300">
          Status
        </div>
        ...
        <div className="mb-2 flex items-center gap-3 rounded-xl border border-white/10 bg-white/5 p-2">
          <span
            className="h-2 w-2 rounded-full"
            style={{ background: "#ffd60a", boxShadow: "0 0 10px #ffd60a" }}
          />
        ...
        <div className="flex items-center gap-3 rounded-xl border border-white/10 bg-white/5 p-2">
          <span
            className="h-2 w-2 rounded-full"
            style={{ background: "#ff006e", boxShadow: "0 0 10px #ff006e" }}
          />
        ...
        {items.map((item) => {
          ...
          return (
            <a
              key={item.id}
              href={`#${item.id}`}
              className="inline-flex items-center gap-2 text-xs md:text-sm uppercase tracking-[0.2em] hover:opacity-100 opacity-80 transition"
