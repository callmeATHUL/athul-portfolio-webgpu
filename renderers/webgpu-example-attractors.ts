...
export async function start(container: HTMLDivElement): Promise<RendererHandle> {
  ...
  const init = tsl.Fn(() => {
    ...
    const basePosition = tsl.vec3(
      tsl.hash(tsl.instanceIndex.add(tsl.uint(Math.random() * 0xffffff))),
      tsl.hash(tsl.instanceIndex.add(tsl.uint(Math.random() * 0xffffff))),
      tsl.hash(tsl.instanceIndex.add(tsl.uint(Math.random() * 0xffffff))),
    )
      .sub(0.5)
      .mul(tsl.vec3(5, 0.2, 5))
    position.assign(basePosition)
    const phi = tsl.hash(tsl.instanceIndex.add(tsl.uint(Math.random() * 0xffffff))).mul(tsl.PI).mul(2)
    const theta = tsl.hash(tsl.instanceIndex.add(tsl.uint(Math.random() * 0xffffff))).mul(tsl.PI)
